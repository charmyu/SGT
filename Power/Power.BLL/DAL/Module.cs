...
namespace Power.DAL
{
	...
	public partial class Module
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Power.Model.Module model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Module(");
			strSql.Append("ID,ModuleName,deviceID,TM,state,templetID,NO)");
			...
			parameters[5].Value = model.templetID;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Power.Model.Module model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public Power.Model.Module GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,ModuleName,deviceID,TM,state,templetID,NO from Module ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,50)			};
			parameters[0].Value = ID;

			Power.Model.Module model=new Power.Model.Module();
		...
		/// 得到一个对象实体
		/// </summary>
		public Power.Model.Module DataRowToModel(DataRow row)
		{
			Power.Model.Module model=new Power.Model.Module();
			if (row != null)
			{
                ...
                {
                    model.templetID = row["NO"].ToString();
                }
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select ID,ModuleName,deviceID,TM,state,templetID,NO ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select m.ID,m.NO,ModuleName,m.TM,m.state,m.templetID,t.templetname ");
            strSql.Append(" From dbo.Module m inner join dbo.Templet t on m.templetID=t.ID ");
            if (strwhere.Trim() != "")
            {
                strSql.Append(" where " + strwhere);
            }
