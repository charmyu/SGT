...
function HuncaSealFile(srcPath,dstPath){
	var netonex=new NetONEX();
	netonex.setupObject();
	var rv;
	try{
    ...
    var result; 
    result=gc.ExtSealFile(srcPath, dstPath);
    if(result!=0){
    	alert("加密失败，失败原因为："+result);
    }
    else{
    	alert("加密成功");
    }
...
function HuncaOpenFile(srcPath,dstPath){
	var netonex=new NetONEX();
	netonex.setupObject();
	var rv;
	try{
    ...
    var result; 
    result=gc.ExtOpenFile(srcPath,dstPath);
    if(result!=0){
    	alert("解密失败，失败原因为："+result);
    }
    else{
    	alert("解密成功");
    }
