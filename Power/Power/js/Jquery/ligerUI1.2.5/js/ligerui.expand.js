...
$.extend($.ligerDefaults.Grid, {
    //modify by zhouyan 2013-09-13，修改了fixedCellHeight，rowHeight，headerRowHeight
    rowHeight: 25,
    checkbox: true,
    ...
    {
        //modify by xufl 2013-06-15
        //LV.tip('发现系统错误 ' + b);
        LV.tip('发生错误,错误码：' + result.status +
...
{ text: '取消', onclick: function (item, dialog) { dialog.hide(); } }
    ...
    function reLoadHistory()
    {
        ...
        var key = getKey();
        var history = LV.cookies.get(key);
        if (history)
        {
    ...
    function removeHistory(name)
    {
        ...
        if (history)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                {
                    data.splice(i, 1);
                    LV.cookies.set(key, JSON2.stringify(data));
                    return;
                }
            }
        }
    ...
    function show()
    {
        ...
        else
        {
            ...
            if (options.search)
            {
                ...
                LV.addSearchButtons(formPanle, grid, containerBtn1, containerBtn2);
            }
            else
            {
                formPanle.remove();
