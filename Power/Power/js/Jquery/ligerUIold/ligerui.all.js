    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        findTextByValue: function (value)
        {
            ...
            var contain = function (checkvalue)
            {
                ...
                {
                    /*update by  zhouyan 2013-7-11 start*/
                    if (targetdata[i] === checkvalue) return true;
                    /*update by  zhouyan 2013-7-11 end*/
                }
                return false;
            };
...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['string'] = function (value, column) {
        ...
        if (column.length && 
            ...
            return value.toString().substr(0, column.length) + "...";
        }
        return value;
    }
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        beginEdit: function (rowParm, containerBulider)
        {
            ...
            //    if (!column.name.toUpperCase() || !column.editor || !column.editor.type || !p.editors[column.editor.type]) continue;
            //    var editor = p.editors[column.editor.type];
            //    var editParm = { record: rowdata, value: rowdata[column.name.toUpperCase()], column: column, rowindex: rowdata['__index'], grid: g };
        ...
        submitEdit: function (rowParm)
        {
            ...
            else
            {
                ...
                for (var columnid in g.editors[rowdata['__id']])
                {
                    ...
                    if (column.name)
                        newdata[column.name.toUpperCase()] = o.editor.getValue(o.input, o.editParm);
                }
        ...
        */
        updateCell: function (arg, value, rowParm)
        {
            ...
            if (value != null && column.name.toUpperCase())
            {
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.name)
            {
                var colName = column.name.toUpperCase();
                jcell.attr({ columnname: colName });
            } 
            var headerText = "";
        ...
        _initBuildGridHeader: function ()
        {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++)
            {
                ...
                $(columns).each(function (i, column)
                {
                    ...
                    {
                        var colwidth = column['_width'];
                        /*add by zhouyan 2013-8-8 start*/
                        if (column._hide) { return;}
                        /*add by zhouyan 2013-8-8 end*/                        
                        if (!column.frozen)
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            //add by zhouyan 2013-07-15 start
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            //Update by daiht 2014-1-16 start
            //重构rowdata对象，增加大写列
            ...
            if (p.tree && (p.tree.columnName != null && p.tree.columnName == column.name.toUpperCase() || p.tree.columnId != null && p.tree.columnId == column.id))
            {
        ...
        _getTotalCellContent: function (column, data)
        {
            ...
            if (column.totalSummary)
            {
                ...
                var max = parseFloat(data[0][column.name.toUpperCase()]);
                var min = parseFloat(data[0][column.name.toUpperCase()]);
                for (var i = 0; i < data.length; i++)
                {
                    count += 1;
                    var value = parseFloat(data[i][column.name.toUpperCase()]);
                    if (!value) continue;
                    sum += value;
        ...
        _onDblClick: function (e)
        {
            ...
            if (src.row)
            {
                //update by zhouyan 2013-7-15 start
                if ($(src.row).hasClass("l-valid")) { return;}
                //update by zhouyan 2013-7-15 end
        ...
        _onClick: function (e)
        {
            ...
            else if (src.hcelltext) //排序
            {
                ...
                var columnName = src.column.name.toUpperCase();
        ...
        select: function (rowParm)
        {
            ...
            //add by daiht 2014-01-14 start
            g._currentSelectedRow = rowdata;
            //add by daiht 2014-01-14 end
...
(function ($)
{
    ...
    $.ligerui.controls.Tip.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            var cornerCls = "l-verify-tip-corner";
            var contentCls = "l-verify-tip-content";
            var tip;
            if (p.isRight == true) {
                cornerCls = cornerCls + "_right";
                contentCls = contentCls + "_right";
                tip = $('<div class="l-verify-tip"><div class="' + contentCls + '"></div><div class="' + cornerCls + '"></div></div>');
            } else {
                tip = $('<div class="l-verify-tip"><div class="' + cornerCls + '"></div><div class="' + contentCls + '"></div></div>');
            }
            ...
            if (p.content)
            {
                $("> ." + contentCls + ":first", tip).html(p.content);
                tip.appendTo('body');
            }
            else
            {
            ...
            p.width && $("> ." + contentCls + ":first", tip).width(p.width - 8);
            p.height && $("> ." + contentCls + ":first", tip).width(p.height);
            eee = p.appendIdTo;
            if (p.appendIdTo)
            {
