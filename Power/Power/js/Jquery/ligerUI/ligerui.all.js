            ...
            create: function (container, editParm, controlOptions)
            {
                ...
                if (editParm.field)
                {
                    ...
                    if (!e.body)
                    {
                        ...
                        //add by xufl 20150715 s
                        if (field.options) {
                            inputBody.attr('options', liger.toJSON(field.options));
                        }
                        if (field.type) {
                            inputBody.attr('ltype', field.type);
                        }
                        if (field.vtype) {
                            inputBody.attr('vtype', field.vtype);
                        }
                        else if (field.type.toLowerCase() == 'date') {
                            inputBody.attr('vtype', 'date');
                        }
                        //add by xufl 20150715 e
                    }
...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            var g = this, p = this.options;
            var r = this._getMatch("yyyy-MM-dd hh:mm:ss");
            if (!r) return null;

            //add by xufl 20150722 s
            dateStr = dateStr.replace("T", " ");
            if (dateStr.length <11) {
                dateStr += " 00:00:00";
            }
            //add by xufl 20150722 d
...
(function ($)
{
    ...
    $.ligerDialog.warning = function (content, title, callback, options)
    {
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.ligerDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.ligerDefaults.DialogString.no, onclick: btnclick, type: 'no' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.ligerDialog.promptDate = function (title, value, multi, callback) {
        var d = new Date();
        ...
        value = curDateTime;
        var target = $('<input type="text" class="l-dialog-inputtext" style="width:200px" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" />');
        if (typeof (multi) == "function") {
            callback = multi;
        }
        if (typeof (value) == "function") {
        ...
        var label = $('<label><label style="color: Red">*</label>发布时间：</label>');
        p = {
            title: title,
            target: label.append(target),
            width: 320,
            buttons: [{ text: $.ligerDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.ligerDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.ligerDialog(p);
...
(function ($)
{
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _initFieldsHtml: function (e)
        {
            ...
            if (fields && fields.length)
            {
                ...
                $(groups).each(function (groupIndex, group)
                {
                    ...
                    {
                        //add by xufl 2013-05-31 start
                        if (!field) return;
                        //add by xufl 2013-05-31 end
        ...
        _buliderLabelContainer: function (field)
        {
            ...
            //modify by xufl 20150720
            //if (label) label += field.rightToken || p.rightToken;
            if (label && label != "~") label += field.rightToken || p.rightToken;
...
(function ($) {
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        //modify by xufl 20150806
        pageStatMessage: '显示从{from}到{to}，每页显示：{pagesize}条，共计 {total} 条。',
    ...
    //add by xufl 2013-06-10
    $.ligerDefaults.Grid.formatters['string'] = function (value, column) {
        if (!value) return "";
        if (column.length &&
            value.toString().length > column.length) {
            return '<span title = "' + value + '">' + value.toString().substr(0, column.length) + "..." + '</span>';
        }
        return value;
    }
    ...
    $.ligerDefaults.Grid.formatters['date'] = function (value, column) {
        ...
        if (typeof (value) == "string" && /^\/Date/.test(value)) {
            ...
            //add by xufl 2013-06-10 start
        else if (typeof (value) == "string") {

            var value = new Date(value.toString().substr(0, 4),//year
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            //modi by xufl 20150806
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:20px;margin:5px' maxlength='3' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadData: function (loadDataParm) {
            ...
            if (p.dataAction == "server") {
                //add code by xufl 2013-05-09 s
                if (p.code) {
                    param.push({ name: "code", value: p.code });
                }
                //add code by xufl 2013-05-09 e
        ...
        loadServerData: function (param, clause) {
            ...
            //add by xufl 20150912 s
            var Code = false; var Page = false; var PageSize = false; var Where = false; var SortName = false; var SortOrder = false;
            for (var key in param) {
                if (!param[key] || !param[key].name) {
                    continue;
                }

                var keyname = param[key].name.toLowerCase();
                if (keyname == "code") {
                    Code = true;
                }
                else if (keyname == "page") {
                    Page = true;
                }
                else if (keyname == "pagesize") {
                    PageSize = true;
                }
                else if (keyname == "where") {
                    Where = true;
                }
                else if (keyname == "sortname") {
                    SortName = true;
            ...
            if (!PageSize) {
                param.push({ name: "pagesize", value: "" });
            }
            if (!Where) {
                param.push({ name: "where", value: "" });
            }
            if (!SortName) {
                param.push({ name: "sortname", value: "" });
            }
            if (!SortOrder) {
                param.push({ name: "sortorder", value: "" });
            }
            //add by xufl 20150912 e
            ...
            var ajaxOptions = {
                ...
                data: param,
                async: p.async,
                //modify by xufl 20150912
                dataType: p.ajaxdatatype ? p.ajaxdatatype : 'json',
                beforeSend: function () {
        ...
        */
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                $(columns).each(function (i, column) {
                    ...
                    if (column['__leaf']) {
                        ...
                        /*add by xufl 2013-8-8 start*/
                        if (column._hide) { return; }
                        /*add by xufl 2013-8-8 end*/
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            //add by xufl 2013-07-15 start
            //if (data.length < p.pageSize) {
            //    var validIndex = data.length;
            //    for (var rows = p.pageSize - data.length; rows > 0; rows--) {
            //        var validHtml = "";
            //        if (validIndex % 2 === 1) {
            //            validHtml = "<tr class='l-grid-row l-grid-row-alt l-valid'>"
            //        } else {
            //            validHtml = "<tr class='l-grid-row l-valid'>"
            //        }
            //        var colLen = g.columns.length;
            //        for (var cols = colLen; cols > 0; cols--) {
            //            if (g.columns[colLen - cols]._hide) {
            //                validHtml = validHtml + "<td class='l-grid-row-cell' style='height:" + p.rowHeight +
            //                "px;display:none;'> </td>";
            //            } else {
            //                validHtml = validHtml + "<td class='l-grid-row-cell' style='height:" + p.rowHeight +
            //                    "px;width:" + g.columns[colLen - cols]._width + "px;'> </td>";
            //            }
            //        }
            //        validHtml = validHtml + "</tr>";
            //        gridhtmlarr.push(validHtml);
            //        validIndex++;
            //    }
            //}
            //add by xufl 2013-07-15 end
        ...
        _getCellContent: function (rowdata, column) {
            ...
            //Update by xufl 2014-1-16 start
            ...
            for (var key in rowdata) {
                ...
                if (key != upperKey) {
                    eval('rowdata.' + upperKey + "= rowdata[key];");
                }
            }

            //var value = column.name ? rowdata[column.name.toUpperCase()] : null;
            var value = g._getValueByName(rowdata, column.name.toUpperCase());
            var text = g._getValueByName(rowdata, column.textField ? column.textField.toUpperCase() : null);

            //Update by xufl 2014-1-16 end
        ...
        _onDblClick: function (e) {
            ...
            var src = g._getSrcElementByEvent(e);
            if (src.row) {
                //update by xufl 2013-7-15 start
                if ($(src.row).hasClass("l-valid")) { return; }
                //update by xufl 2013-7-15 end
        ...
        _onClick: function (e) {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly) {
                    ...
                    //update by xufl 2013-7-15 start
                    if (row.hasClass("l-valid")) { return; }
                    //update by xufl 2013-7-15 end
        ...
        select: function (rowParm) {
            ...
            //add by xufl 2014-01-14 start
            g._currentSelectedRow = rowdata;
            //add by xufl 2014-01-14 end
