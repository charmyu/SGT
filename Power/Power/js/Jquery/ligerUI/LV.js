(function ($)
{
    ...
    LV.ajax = function (options, callback) {
       
        var p = options || {};
        if (!p.url) {
            //LV.showError("ajax 请求地址不能为空.");
            LV.tip("ajax 请求地址不能为空.");
        }
        ...
        $.ajax({
            ...
            error: function (result, b, c) {
                if (p.error)
                    ...
                    //LV.tip('请求发生错误 <BR>错误码：' + result.status +'<BR>错误信息：' + result.statusText + ',' + result.responseText);
                    LV.tip('错误信息：' + result.responseText);
            }
        });
    };
    ...
    LV.bulidFilterGroup = function (form)
    {
        ...
        .each(function ()
        {
            ...
            //modify by xufl 2015-07-15
            //var type = $(this).attr("vt") || "string";
            var type = $(this).attr("vtype") || "string";
    ...
    LV.appendResetButtons = function (form, grid) {
        ...
        LV.addResetButtons(form, grid, container.find("li:eq(0)"));
    };
    ...
    LV.f_setFormBtn = function (buttons, action) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if ((buttons[i].action + "") == "undefined" || $.trim(buttons[i].action) == "") {
        ...
        var p = $.extend({
            ...
            success: function (data) {
                ...
                for (var i in lstele) {
                    ...
                    //处理数据的大小写
                    for (var p in data) {
                        if ( p && lstele[i].name.toString().toUpperCase() == p.toUpperCase()) {
                            if (p != lstele[i].name) {
                                eval('data.' + lstele[i].name + "= data[p];");
                            }
                        }
                    }

                    var ele = $("[name=" + (preID + lstele[i].name) + "]", mainform);
                    //针对复选框和单选框 处理
                    if (ele.is(":checkbox,:radio")) {
                        ele[0].checked = (data[lstele[i].name] + "" == ele[0].value) ? true : false;
                    }
                    else {
                        ele.val(data[lstele[i].name]);
                    }
                }
                //modify by xufl 2015-09-02 e
            ...
            error: function (result) {
                ...
                //LV.showError('<b>请求发生错误</b>：' + result.responseText);
                LV.tip('请求发生错误：' + result.responseText);
            }
        }, options);
        LV.ajax(p);
    };
    ...
    LV.submitForm = function (mainform, success, error)
    {
        ...
        if (mainform.valid())
        {
            mainform.ajaxSubmit({
                ...
                error: function (result)
                {
                    //update by zhouyan 2015-1-21 end
    ...
    LV.validate = function (form, options) {
        ...
        options = $.extend({
            errorPlacement: function (lable, element) {
                ...
                ele.find(".validateimg").css("background-position", "-60px 0");
                switch (msgType) {
                    case "msgtext":
                        //modify by xufl 20150731
                        //var msgControl = ele.find("#" + element.attr("name") + "_msg")
                        //if (msgControl) {
                        //    msgControl.html(errMsg);
                        //}
                        //lable.appendTo(element.parent());

                        msgHtml = "<p id ='" + element.attr("name") + "_msg_p' style='clear:both;'><span style='margin-top:17px;'><span class='validateimg'></span><span id='"
                                        + element.attr("name") + "_msg' style='line-height: 25px; color: red; float:left'>" + errMsg + "</span></span></p>";

                        //msgHtml = "<p id ='" + element.attr("name") + "_msg_p'><span id='" + element.attr("name") + "_msg' style='line-height: 25px; color: red;'>" + errMsg + "</span></p>";

                        $("#" + element.attr("name") + "_msg_p").remove();                        
                        $(msgHtml).appendTo($('[name=' + element.attr("name") + ']').parent());
                    ...
                    case "tip":
                        ...
                        break;
                    default:
                        var tipEle = ele.find(".validateimg");
            ...
            success: function (lable) {
                //start modify by chenlong 2015/8/3  
                //var element = $("[name='" + lable.attr("for") + "']") || $("[name='" + lable.attr("for") + "']");
                var element = $("#" + lable.attr("for"));
                //end modify by chenlong

                if (!lable.attr("for")) return;
                if (msgType != "tip") {
                    //var ele = element.next();
                    //ele.find(".validateimg").css("background-position", "-30px 0");
    ...
    //打开Tab项
    LV.openTab = function (title, url) {
        //if (!tabid) {
        //    tabid = $("#framecenter > .l-tab-content > .l-tab-content-item:visible").attr("tabid");
        //}
        var tab = tab ? tab : window.parent.tab;
        tab = tab ? tab : window.parent.parent.tab;
        tab = tab ? tab : window.parent.parent.parent.tab;
        if (tab) {
            tab.addTabItem({ tabid: str2asc(url), text: title, url: url });
        }
    };
    
    //关闭Tab项,如果url不指定，那么关闭当前显示的
    LV.closeTab = function (url) {
    ...
    //刷新tab
    LV.reloadTab = function (url, isCloseCurrentTab) {
        var tabid = str2asc(url);
        ...
        var iframe = window.frames[tabid] ? window.frames[tabid] : window.parent.frames[tabid];
    ...
    //打开窗体
    LV.openWindow = function (url, width, height, title) {

        var windowOpened = null;
        try {
            windowOpened = $.ligerDialog.open({
                url: url,
                title: title, cls: "field", height: height, width: width, name: '',
                isResize: true
            });
        }
        catch (e) {
...
function str2asc(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        result = result + chr.charCodeAt(0);
    }
    return result;
}